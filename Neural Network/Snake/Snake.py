...
def random_brain():
    Lay1 = Layer_Dense(24,32)
    Lay2 = Layer_Dense(32,32)
    Lay3 = Layer_Dense(32,4)
    Brain = [Lay1,Lay2,Lay3]
    return Brain

def run_brain(Brain,inputs):
    Brain[0].forward(inputs)
...
def averaged_sim(pop,gens): #runs each brain x times and takes an average
    ...
    def avg_run(brain):
        ...
        all_runs = [run_game(brain) for i in range(runs)]
...
def save_brain(brain,name):
    #use this save_matrix(brain[0],name+'_lay1')
    save_matrix(brain[0].weights,name+'_lay1')
    save_matrix(brain[0].biases,name+'_add1')
    save_matrix(brain[1].weights,name+'_lay2')
    save_matrix(brain[1].biases,name+'_add2')
    save_matrix(brain[2].weights,name+'_lay3')
    save_matrix(brain[2].biases,name+'_add3')

def open_brain(name):
    brain = random_brain()
    
    brain[0].weights = open_matrix(name+'_lay1')
    brain[0].biases = open_matrix(name+'_add1')
    brain[1].weights = open_matrix(name+'_lay2')
    brain[1].biases = open_matrix(name+'_add2')
    brain[2].weights = open_matrix(name+'_lay3')
    brain[2].biases = open_matrix(name+'_add3')
